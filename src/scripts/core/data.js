...
const TOURS = [
  {
    ...
    venues: [
      ...
      { name:"Shakespeare & Sons", type:"English Bookshop", addr:"Krymská 12, Praha 10", what:"Beloved independent bookshop. Best selection of Prague literature in English. Staff picks are excellent.", tip:"Check their events calendar: readings happen regularly in the back room.", icon:'<lord-icon src="assets/icons/lordicon/kipaqhoz.json" trigger="hover" colors="primary:#C9A84C" style="width:20px;height:20px"></lord-icon>' },
  ...
  },
  {
    ...
    venues: [
      ...
      { name:"Prague Jewish Town Hall", type:"Cultural Site", addr:"Maiselova 18, Praha 1", what:"The only Jewish Town Hall in the world with two clocks: one Hebrew (running right to left), one Czech. A symbol of two identities coexisting.", tip:"The clock tower is visible from the street: look for it above the entrance.", icon:'<lord-icon src="assets/icons/lordicon/abgtphux.json" trigger="hover" colors="primary:#C9A84C" style="width:20px;height:20px"></lord-icon>' },
  ...
  },
  {
    ...
    stops: [
      { n:"Old Town Square: The Magic Clock", a:"Staroměstské nám.", dur:"25 min",
        ...
        photo:"assets/extras/clock-tower.jpg",
...
const EXTRAS = [
  // Food & Drink
  { id:"beer-tour",     cat:"Food and Drink",  name:"Czech Food and Beer Tour",          desc:"Pub crawl through 3 historic Prague breweries with traditional Czech bites at each stop.",                    price:45, img:"assets/extras/beer-tour.jpg" },
  { id:"beer-spa",      cat:"Food and Drink",  name:"Beer Spa Experience",             desc:"Soak in a private oak barrel of hops & malt with unlimited cold beer from your own tap.",                   price:65, img:"assets/extras/beer-spa.jpg" },
  { id:"medieval",      cat:"Food and Drink",  name:"Medieval Dinner Show",            desc:"5-course feast with unlimited beer, fire dancers, jugglers, and swordfighting, a full spectacle.",          price:55, img:"assets/extras/medieval-dinner.jpg" },
  { id:"cooking-class", cat:"Food and Drink",  name:"Czech Cooking Masterclass",       desc:"Private cooking class with a Czech chef: svíčková, trdelník, and traditional dumplings from scratch.",    price:75, img:"assets/extras/cooking-class.jpg" },
  { id:"wine-cellar",   cat:"Food and Drink",  name:"Moravian Wine Cellar Tasting",    desc:"Guided tasting of 8 premium Moravian wines in a historic underground cellar with a sommelier.",           price:55, img:"assets/extras/wine-cellar.jpg" },
  // River & Water
  { id:"river-cruise",  cat:"River and Water", name:"Vltava Dinner Cruise",            desc:"2-hour scenic dinner cruise with live jazz, sunset views, and full Czech dinner service.",                   price:50, img:"assets/extras/river-cruise.jpg" },
  { id:"beer-bike",     cat:"River and Water", name:"Swimming Beer Bike",              desc:"Pedal-powered party boat on the Vltava, unlimited draft beer, great views, great laughs.",                 price:35, img:"assets/extras/beer-bike.jpg" },
  { id:"car-boat",      cat:"River and Water", name:"Antique Car Boat Ride",           desc:"Vintage-style car boat cruise past Prague's landmarks, charming and completely unique.",                   price:30, img:"assets/extras/car-boat.jpg" },
  { id:"paddle",        cat:"River and Water", name:"Paddle Boating",                  desc:"Relaxing paddle boat on the Vltava, unique perspective on Charles Bridge and the Castle.",                  price:15, img:"assets/extras/paddle-boat.jpg" },
  // Cultural
  { id:"castle-skip",   cat:"Cultural",      name:"Prague Castle Skip-the-Line",     desc:"VIP fast-track entry to the world's largest ancient castle complex. No queuing, ever.",                     price:25, img:"assets/extras/castle-skip.jpg" },
  { id:"clock-tower",   cat:"Cultural",      name:"Astronomical Clock Tower",        desc:"Private after-hours climb of the clock tower for panoramic views of Old Town Square at dusk.",              price:22, img:"assets/extras/clock-tower.jpg" },
  { id:"underground",   cat:"Cultural",      name:"Underground Prague",              desc:"Explore medieval cellars, tunnels, and secret corridors beneath Old Town, 700 years of history below.",   price:30, img:"assets/extras/underground.jpg" },
  { id:"communist",     cat:"Cultural",      name:"Communist Prague & Nuclear Bunker",desc:"Cold War tour: secret police cells, Velvet Revolution memorial & an actual nuclear bunker.",               price:35, img:"assets/extras/communist-tour.jpg" },
  { id:"concert",       cat:"Cultural",      name:"Classical Concert: Municipal House",desc:"Live symphony orchestra in Prague's stunning Art Nouveau concert hall. Dress code: smart.",              price:40, img:"assets/extras/concert.jpg" },
  { id:"opera",         cat:"Cultural",      name:"Opera at the State Opera",        desc:"World-class opera in a gorgeous neo-Rococo venue. Subtitles in 3 languages.",                               price:35, img:"assets/extras/opera.jpg" },
  { id:"lennon",        cat:"Cultural",      name:"Lennon Wall and Street Art Tour",   desc:"Guided walk through Prague's most expressive street art scene, the story of rebellion in colour.",         price:20, img:"assets/extras/lennon-wall.jpg" },
  { id:"vysehrad",      cat:"Cultural",      name:"Vyšehrad Fortress Walk",          desc:"Panoramic views, ancient basilica, and the cemetery where Czech legends are buried.",                       price:20, img:"assets/extras/vysehrad.jpg" },
  { id:"glassblowing",  cat:"Cultural",      name:"Czech Glass Blowing Workshop",    desc:"Create your own glass piece with a master glassblower in a Bohemian studio. Keep your creation.",         price:55, img:"assets/extras/glassblowing.jpg" },
  { id:"photoshoot",    cat:"Cultural",      name:"Old Town Photography Session",    desc:"2-hour shoot with a professional photographer in Prague's most photogenic locations, at golden hour.",    price:120, img:"assets/extras/photoshoot.jpg" },
  // Day Trips
  { id:"bohemia",       cat:"Day Trips",     name:"Bohemian Switzerland",            desc:"Full-day hike: Pravčická Gate arch, Edmund Gorge boat ride, pine forest trails.",                           price:85, img:"assets/extras/bohemia.jpg" },
  { id:"krumlov",       cat:"Day Trips",     name:"Český Krumlov",                   desc:"UNESCO fairy-tale castle town: castle tour, river rafting, medieval tavern lunch.",                        price:75, img:"assets/extras/krumlov.jpg" },
  { id:"kutna",         cat:"Day Trips",     name:"Kutná Hora & Bone Church",        desc:"Sedlec Ossuary (the famous bone church), St. Barbara's Cathedral, silver mining history.",                 price:60, img:"assets/extras/kutna-hora.jpg" },
  { id:"karlovy",       cat:"Day Trips",     name:"Karlovy Vary Spa Town",           desc:"Hot springs, Moser crystal glass factory, Becherovka distillery tasting, thermal spa.",                   price:70, img:"assets/extras/karlovy-vary.jpg" },
  // Adventure
  { id:"segway",        cat:"Adventure",     name:"Segway / E-Scooter Tour",         desc:"Glide through Prague's hidden districts and riverside paths, away from tourist crowds.",                  price:40, img:"assets/extras/segway.jpg" },
  { id:"ebike",         cat:"Adventure",     name:"E-Bike River Trail",              desc:"Scenic cycling along the Vltava with stops at local riverside beer gardens.",                               price:35, img:"assets/extras/ebike.jpg" },
  { id:"helicopter",    cat:"Adventure",     name:"Helicopter Tour of Prague",       desc:"15-minute private helicopter flight over Prague Castle, Old Town, and the Vltava. Unforgettable.",        price:220, img:"assets/extras/helicopter.jpg" }
];
...
const AFTER_DARK = {
  jazz: [
    { name:"Jazz Dock", addr:"Janáčkovo nábřeží 2, Praha 5", type:"Jazz Club", desc:"Prague's finest jazz club, built directly on the Vltava riverbank. Live jazz nightly from 9pm. Book the riverside table for the best view.", price_from:15, img:"assets/venues/jazz-club.jpg", tip:"The whisky selection is exceptional: 80+ single malts. Arrive by 8:30pm for the best seats." },
    { name:"AghaRTA Jazz Centrum", addr:"Železná 16, Praha 1", type:"Jazz Club", desc:"Old Town's legendary jazz basement. Running since 1991, it's a Prague institution. Traditional and fusion jazz every night from 9pm.", price_from:12, img:"assets/venues/jazz-club.jpg", tip:"The Thursday night jam sessions are legendary. Check the website for headline acts." },
    { name:"Reduta Jazz Club", addr:"Národní 20, Praha 1", type:"Jazz Club", desc:"Prague's oldest jazz club, opened in 1958. President Clinton played here in 1994. Historic, intimate, unmissable.", price_from:14, img:"assets/venues/jazz-club.jpg", tip:"Book online: it sells out. The Dixieland nights on Fridays are extraordinary." }
  ],
  rooftop: [
    { name:"T-Anker Rooftop Bar", addr:"nám. Republiky 8, Praha 1", type:"Rooftop Bar", desc:"360° panoramic view of Old Town from a rooftop terrace. Best sunset view in the city. Czech craft beers and creative cocktails.", price_from:12, img:"assets/venues/rooftop-bar.jpg", tip:"Go at 7pm for golden hour. The Municipal House dome is directly opposite." },
    { name:"Cloud 9 Sky Bar and Lounge", addr:"Pobřežní 1, Praha 8 (Hilton)", type:"Sky Bar", desc:"Hilton's rooftop bar, sophisticated, quiet, and with arguably the best view of the Vltava from any bar in Prague.", price_from:15, img:"assets/venues/rooftop-bar.jpg", tip:"Dress code applies after 9pm. The espresso martini is their signature." },
    { name:"Estrella Sky Bar", addr:"Zlatnická 12, Praha 1", type:"Rooftop Bar", desc:"Open-air rooftop with a view over Charles Bridge and Prague Castle. Fire pits in winter. One of the most romantic bars in Europe.", price_from:14, img:"assets/venues/rooftop-bar.jpg", tip:"Book the VIP section for groups of 4+. The table service is excellent." }
  ],
  cocktail_bars: [
    { name:"Hemingway Bar", addr:"Karoliny Světlé 26, Praha 1", type:"Cocktail Bar", desc:"Prague's most celebrated cocktail bar. Pre-Prohibition recipes, fresh ingredients, and an atmosphere that feels genuinely 1930s. Flawless.", price_from:16, img:"assets/venues/cocktail-bar.jpg", tip:"No reservations: queue is worth it. The Hemingway Daiquiri is non-negotiable. Ask the barman about the gin collection." },
    { name:"Black Angel's Bar", addr:"Staroměstské nám. 29, Praha 1", type:"Cocktail Bar", desc:"Hidden underground bar beneath the Old Town. Gothic arches, candlelight, and some of the most creative cocktails in Central Europe. Theatrical.", price_from:14, img:"assets/venues/cocktail-bar.jpg", tip:"Try the Absinthe ceremony: a genuine Czech absinthe ritual performed tableside." },
    { name:"Banker's Bar", addr:"Senovážné nám. 5, Praha 1", type:"Cocktail Bar", desc:"1920s banking hall converted into a bar of extraordinary elegance. Marble columns, brass fittings, and a cocktail menu that changes monthly.", price_from:15, img:"assets/venues/cocktail-bar.jpg", tip:"The bar food is exceptional: the wagyu sliders at midnight are remarkable." }
  ],
  shows: [
    { name:"Czech Medieval Tavern", addr:"Mostecká 21, Praha 1", type:"Dinner Show", desc:"Theatrical 5-course medieval feast with fire eaters, jesters, live music and sword fights. Unlimited mead and beer. A complete spectacle.", price:55, img:"assets/extras/medieval-dinner.jpg", tip:"Book the VIP table near the stage for the best show. Vegetarian menu available on request." },
    { name:"Black Light Theatre Image", addr:"Pařížská 4, Praha 1", type:"Theatre", desc:"Prague's world-famous Black Light Theatre, an optical illusion performance unlike anything else in Europe. No language barrier. All ages.", price:28, img:"assets/venues/black-light-theatre.jpg", tip:"Sit in the centre of the stalls, not the sides. The 3D effects only work from the front." }
  ]
};
